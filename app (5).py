...
def predict_all(hba, hbd, ratio, temp_K, water):
    ...
    for target, (model, log_t) in models.items():
        ...
        val = np.expm1(val) if log_t else val
        # Redox potential is always negative (-1.9 to -0.9 V) — do NOT clamp to 0
        if target == 'reduction_potential_V_vs_SHE':
            preds[target] = round(float(val), 4)
        else:
            preds[target] = max(0, round(float(val), 4))
    return preds, None
...
def predict_li_recovery(preds, temp_K, source, oxidant, assist, sl_ratio, pretreat, leach_time):
    ...
    redox_factor = 1.0 + max(0, -redox - 0.9) * 0.04  # bonus for redox < -0.9 V
...
def sweep_temps(hba, hbd, ratio, water):
    ...
    for prop in ['viscosity_cP', 'pH_acidity', 'density_kg_m3', 'li_extraction_score']:
        ...
        if prop == 'reduction_potential_V_vs_SHE':
            results[prop] = [float(v) for v in vals]
        else:
...
def build_li_tips(preds, temp_K, source, oxidant, assist, pretreat):
    ...
    if redox > -1.0:
        tips.append(f"⚠️ **Reduction potential weak ({redox:.2f} V vs SHE)** — add H₂O₂ or ascorbic acid to reduce Co³⁺→Co²⁺ and release bound Li⁺.")
    elif redox < -1.5:
...
# ════════ NEW — PROCESS OPTIMISER & DEEP ANALYSIS (still inside Tab 1) ════════
    ...
    ana_tabs = st.tabs(["⚙️ Process Window","🗺️ Condition Heatmap",
    ...
    with ana_tabs[4]:
        ...
        disp['Li Recovery'] = disp['Li Recovery'].apply(lambda v: f"{'✅' if v>=90 else '⚠️' if v>=75 else '❌'} {v:.1f}%")
        disp['Viscosity (cP)'] = disp['Viscosity (cP)'].apply(lambda v: f"{'🟢' if v<100 else '🟡' if v<300 else '🔴'} {v:.0f}")
        disp['pH'] = disp['pH'].apply(lambda v: f"{'🟢' if v<3 else '🟡' if v<5 else '🔴'} {v:.2f}")
        disp['Li Score'] = disp['Li Score'].apply(lambda v: f"{'🟢' if v>=70 else '🟡' if v>=50 else '🔴'} {v:.1f}")
...
with tabs[1]:
    ...
    with col2:
        st.markdown("#### Model accuracy & training stats")
        perf_rows = [
            {"Property":"Viscosity (cP)",       "R²":0.8959,"MAE":"315 cP",    "Train N":4920,"Status":"✅"},
            {"Property":"Density (kg/m³)",       "R²":0.9948,"MAE":"1.04 kg/m³","Train N":1264,"Status":"✅"},
            {"Property":"pH",                    "R²":0.9888,"MAE":"0.076",     "Train N":1005,"Status":"✅"},
            {"Property":"Redox potential (V)",   "R²":0.9729,"MAE":"0.029 V",   "Train N":1737,"Status":"✅"},
            {"Property":"Li extraction score",   "R²":0.8679,"MAE":"1.94%",     "Train N":878, "Status":"🔋"},
        ]
        ...
        if ph_pred < 2.5:
            interp.append("⚗️ **Strongly acidic** — will rapidly dissolve Li₂CO₃ and LiCoO₂ oxide phases.")
        elif ph_pred < 4:
            interp.append("⚗️ **Mildly acidic** — will dissolve most Li phases. Good for selective Li leaching.")
        else:
            interp.append("⚗️ **Insufficient acidity** — Li oxide dissolution will be slow. Switch to oxalic or malonic acid HBD.")
        if redox_pred < -1.3:
        ...
        for item in interp:
            st.markdown(f'<div class="tip-box" style="margin-bottom:6px">{item}</div>', unsafe_allow_html=True)
...
with tabs[2]:
    ...
    <div style="background:#1e293b;border:1px solid #334155;border-radius:10px;padding:14px 18px;margin-bottom:14px">
      <span style="color:#60a5fa;font-weight:600">New Li features:</span>
      <span style="color:#94a3b8"> Two binary flags —
      <b>Is_Li_HBA</b> (1 if HBA is in 11 literature-curated Li compounds) and
      <b>Is_Li_HBD</b> (1 if HBD is in 14 Li-relevant compounds) — were added to all 5 models.
      Their importance shows how much molecular identity beyond the encoder label matters for Li prediction.</span>
    </div>
    """, unsafe_allow_html=True)
...
with tabs[6]:
    ...
    <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:10px;margin-bottom:16px">
      <div style="background:#1e293b;border:1px solid #334155;border-radius:10px;padding:12px;text-align:center">
        ...
        <div style="font-size:26px;font-weight:700;color:#f1f5f9">5,790</div>
      </div>
      <div style="background:#1e293b;border:1px solid #334155;border-radius:10px;padding:12px;text-align:center">
        <div style="font-size:10px;color:#94a3b8;font-weight:600;margin-bottom:4px">MODELS</div>
        <div style="font-size:26px;font-weight:700;color:#60a5fa">5</div>
        <div style="font-size:11px;color:#64748b">XGBoost</div>
      </div>
      <div style="background:#1e293b;border:1px solid #334155;border-radius:10px;padding:12px;text-align:center">
        <div style="font-size:10px;color:#94a3b8;font-weight:600;margin-bottom:4px">UNIQUE HBAs</div>
        <div style="font-size:26px;font-weight:700;color:#f1f5f9">144</div>
      </div>
      <div style="background:#1e293b;border:1px solid #334155;border-radius:10px;padding:12px;text-align:center">
        <div style="font-size:10px;color:#94a3b8;font-weight:600;margin-bottom:4px">UNIQUE HBDs</div>
        <div style="font-size:26px;font-weight:700;color:#f1f5f9">167</div>
      </div>
      <div style="background:#0f3460;border:1px solid #1d4ed8;border-radius:10px;padding:12px;text-align:center">
        <div style="font-size:10px;color:#93c5fd;font-weight:600;margin-bottom:4px">Li FEATURES</div>
        <div style="font-size:26px;font-weight:700;color:#60a5fa">2</div>
        <div style="font-size:11px;color:#93c5fd">new flags</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
    st.divider()
...
|---|---|---|---|---|
| Viscosity (cP) | 0.8959 | 315 cP | 4,920 | log(1+x) transform |
| Density (kg/m³) | 0.9948 | 1.04 kg/m³ | 1,264 | |
| pH | 0.9888 | 0.076 | 1,005 | |
| Redox potential (V) | 0.9729 | 0.029 V | 1,737 | always negative: −1.9 to −0.9 |
| **Li extraction score** | **0.8679** | **1.94%** | **878** | **NEW** |

**Algorithm:** XGBoost · n_estimators=400 · max_depth=7 · lr=0.04 · subsample=0.8  
**Features (7):** HBA enc · HBD enc · molar ratio · temperature · water · Is_Li_HBA · Is_Li_HBD  
**Split:** 85% train / 15% test · random_state=42  
**Redox note:** Values are always negative (−1.9 to −0.9 V vs SHE). More negative = stronger reducing = better Li release.
        """)

        st.markdown("#### Li extraction thresholds")
        rows = [
            ("Viscosity",       "< 200 cP",     "Li⁺ diffusion coefficient"),
            ("pH",              "1.5 – 3.5",    "Dissolves Li₂CO₃ and LiCoO₂"),
            ("Redox potential", "< −1.0 V",     "Reduces Co³⁺→Co²⁺, releases Li⁺"),
            ("Temperature",     "60 – 90 °C",   "Kinetics vs DES stability trade-off"),
    ...
    with col2:
        ...
        <div style="background:#1e293b;border:1px solid #334155;border-radius:10px;padding:14px">
          <div style="color:#94a3b8;font-size:12px;margin-bottom:8px">Files required on Streamlit Cloud</div>
          <div style="font-family:monospace;font-size:13px">
            <div style="color:#60a5fa">📄 app.py</div>
            <div style="color:#34d399">📦 model_data.py <span style="color:#64748b">(6.5 MB — all 5 models embedded)</span></div>
            <div style="color:#fbbf24">📋 requirements.txt</div>
          </div>
          <div style="color:#64748b;font-size:11px;margin-top:10px">
            No pkl/json model files needed. Models are base64-encoded inside model_data.py.
          </div>
        </div>
        """, unsafe_allow_html=True)

        st.markdown("#### Citation")
        st.code("Moradi, F. & Bougie, F. (2026).\nJ. Mol. Liq. 443, 128903.\nhttps://doi.org/10.1016/j.molliq.2025.128903", language="text")
